...
const MessageItem = ({ message }: { message?: MessageType }) => {
  ...
  const time = new Date(message?.createdAt as string).toLocaleTimeString(
    "en-US",
    {
      hour: "numeric",
      minute: "numeric",
    ...
    return (
      <div className="place-self-end flex gap-2 mb-2 justify-self-end">
        <div className="bg-[#095699] text-white py-1 px-4 rounded-xl flex items-end gap-3">
          <p>{message.content}</p>
          <p className="text-[10px] font-bold text-gray-200">{time}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex gap-2 mb-2">
      ...
      <div className="bg-white text-black py-1 px-4 rounded-xl flex items-end gap-3">
        <p>{message?.content}</p>
        <p className="text-[10px] font-bold text-black">{time}</p>
      </div>
    </div>
  );
};
